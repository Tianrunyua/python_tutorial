...
Print the letter SUM (uppercase) with a blank space before and after the equal signal followed by the sum of the N numbers,  and letter PRODUCT (uppercase) with a blank space before and after the equal signal followed by the sum of the M integer numbers,  according to the following example.  
Samples Input：  
5  
10.0  
9.2  
